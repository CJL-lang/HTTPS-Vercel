    ...
    useEffect(() => {
        const fetchRecords = async () => {
            ...
            try {
                ...
                if (!res.ok) {
                    const text = await res.text().catch(() => '');
                    throw new Error(`Fetch skills assessments failed: ${res.status} ${res.statusText} ${text.slice(0, 200)}`);
                }
                const data = await parseJsonResponse(res, 'Fetch skills assessments');
                // 映射后端字段到前端格式
