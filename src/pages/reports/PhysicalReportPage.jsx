    ...
    useEffect(() => {
        const fetchRecords = async () => {
            ...
            try {
                ...
                const res = await fetch(`/api/assessments/${targetId}?type=0`, {
                    headers: { 'Authorization': `Bearer ${user.token}` }
                });
                if (!res.ok) {
                    const text = await res.text().catch(() => '');
                    throw new Error(`Fetch physical assessments failed: ${res.status} ${res.statusText} ${text.slice(0, 200)}`);
                }
                const data = await parseJsonResponse(res, 'Fetch physical assessments');
                // 映射后端字段到前端格式
